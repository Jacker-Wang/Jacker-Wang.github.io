...
```
启动三台机器上的mongo实例，
`mongod -f $MONGODB_HOME/conf/config.cfg`</br>
...
每个分片服务都是有3台机器上的复制集组成，所以以下每个分片服务的配置要在三台机器上进行相同的操作</br>
### 第一个分片服务器的搭建
...
```
然后启动每个机器上的分片服务，
`mongod -f $MONGODB_HOME/conf/shared1.cfg`</br>
登录任意分片服务器，进行初始化
```
#mongo --port 27001
config={
...
```
然后启动每个机器上的分片服务，
`mongod -f $MONGODB_HOME/conf/shared2.cfg`</br>
登录任意分片服务器，进行初始化
...
然后启动每个机器上的分片服务，
`mongod -f $MONGODB_HOME/conf/shared3.cfg`</br>
登录任意分片服务器，进行初始化
```
#mongo --port 27003
config={
